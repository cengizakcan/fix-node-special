...
class NodeRtmpSession {
  ...
  onPublish(invokeMessage) {
	...
	var thisIs = this;
	  ...
	  NodeCoreUtils.verifyAuth(this.publishArgs.sign, this.publishStreamPath, this.config.auth.secret).then(function(result){  
	  
		  if (result === "error") {
			Logger.log(`[rtmp publish] Unauthorized. id=${thisIs.id} streamPath=${thisIs.publishStreamPath} streamId=${thisIs.publishStreamId} sign=${thisIs.publishArgs.sign} `);
			thisIs.sendStatusMessage(thisIs.publishStreamId, 'error', 'NetStream.publish.Unauthorized', 'Authorization required.');
			return;
		  }
	  
	      if (context.publishers.has(thisIs.publishStreamPath)) {
		  Logger.log(`[rtmp publish] Already has a stream. id=${thisIs.id} streamPath=${thisIs.publishStreamPath} streamId=${thisIs.publishStreamId}`);
		  ...
		  Logger.log(`[rtmp publish] NetConnection is publishing. id=${thisIs.id} streamPath=${thisIs.publishStreamPath} streamId=${thisIs.publishStreamId}`);
		  thisIs.sendStatusMessage(thisIs.publishStreamId, 'error', 'NetStream.Publish.BadConnection', 'Connection already publishing');
		} else {
		  Logger.log(`[rtmp publish] New stream. id=${thisIs.id} streamPath=${thisIs.publishStreamPath} streamId=${thisIs.publishStreamId}`);
		  context.publishers.set(thisIs.publishStreamPath, thisIs.id);
		  ...
		  setTimeout(() => {
			thisIs.publishArgs.ac = thisIs.audioCodec;
			thisIs.publishArgs.vc = thisIs.videoCodec;
			context.nodeEvent.emit('postPublish', thisIs.id, thisIs.publishStreamPath, thisIs.publishArgs);
		  }, 200);
